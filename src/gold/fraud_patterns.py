...
class FraudDetector:
    ...
    def detect_all(self, target_date: datetime | None = None) -> DataFrame:
        ...
        day_data = silver.filter(
            (F.col("_date") == target_date.date()) & (F.col("_quality_flag") == "PASS")
        )
        ...
        # Combine all flags
        all_flags = (
            velocity_flags.unionByName(amount_flags, allowMissingColumns=True)
    ...
    def _detect_velocity_anomaly(self, day_data: DataFrame, full_silver: DataFrame) -> DataFrame:
        """Detect senders with unusually high transaction volume."""
        sender_counts = (
            day_data.groupBy("sender_phone")
            .agg(F.count("*").alias("daily_txn_count"))
            .filter(F.col("daily_txn_count") > VELOCITY_THRESHOLD)
        )

        return day_data.join(sender_counts, "sender_phone").select(
            "transaction_id",
            "sender_phone",
            "recipient_phone",
            "amount",
    ...
    def _detect_amount_spikes(self, day_data: DataFrame, full_silver: DataFrame) -> DataFrame:
        """Detect transactions significantly above sender's 7-day average."""
        seven_days_ago = datetime.utcnow() - timedelta(days=7)
        historical = full_silver.filter(
            (F.col("initiated_at") >= seven_days_ago) & (F.col("_quality_flag") == "PASS")
        )

        sender_avg = historical.groupBy("sender_phone").agg(F.avg("amount").alias("avg_amount_7d"))

        return (
            day_data.join(sender_avg, "sender_phone")
    ...
    def _detect_rapid_transfers(self, day_data: DataFrame) -> DataFrame:
        ...
        window_spec = Window.partitionBy("sender_phone", "recipient_phone").orderBy("initiated_at")

        rapid = (
            day_data.filter(F.col("transaction_type") == "TRANSFER")
            .withColumn("prev_txn_time", F.lag("initiated_at").over(window_spec))
            .withColumn(
                "minutes_since_prev",
                F.when(
                    F.col("prev_txn_time").isNotNull(),
                    (F.unix_timestamp("initiated_at") - F.unix_timestamp("prev_txn_time")) / 60,
                ),
            )
    ...
    def _detect_off_hours_large(self, day_data: DataFrame) -> DataFrame:
        ...
        off_hours = day_data.filter(
            (~F.col("_is_peak_hour")) & (F.col("amount") >= OFF_HOURS_LARGE_AMOUNT)
        )
